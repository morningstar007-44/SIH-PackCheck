...
export class OpenAIOCRService implements OCRService {
  ...
  private fallbackService: TesseractOCRService;

  constructor() {
    this.apiKey = import.meta.env.VITE_OPENAI_API_KEY || '';
    this.fallbackService = new TesseractOCRService();
  }

  async extractText(image: File | Blob): Promise<OCRResult> {
    if (!this.apiKey) {
      console.warn('OpenAI API key missing. Falling back to local Tesseract OCR.');
      return this.fallbackService.extractText(image);
    }
    ...
    try {
      ...
      if (!response.ok) {
        ...
        console.warn(`OpenAI Vision API error (${response.status}): ${err.error?.message || response.statusText}. Falling back to Tesseract OCR.`);
        return this.fallbackService.extractText(image);
      }
      ...
      console.warn('OpenAI Vision OCR execution failed. Falling back to local Tesseract OCR:', err);
      return this.fallbackService.extractText(image);
    }
  }
}
